...
class Currency(object):
    ...
    def __request(symbol):
        response = YQLQuery().execute('select * from yahoo.finance.xchange where pair = "{0}"'.format(symbol))
        try:
...
class Share(object):
    ...
    @staticmethod
    def __request(symbol):
        response = YQLQuery().execute('select * from yahoo.finance.quotes where symbol = "{0}"'.format(symbol))
        try:
            return response['query']['results']
        except KeyError:
            try:
    ...
    def __request_information(symbol):
        response = YQLQuery().execute('select * from yahoo.finance.stocks where symbol = "{0}"'.format(symbol))
        try:
            return response['query']['results']
        except KeyError:
            try:
